...
constraint_lb <- c(1.0, 1.0, 1.0)
constraint_ub <- c(1.0, 1.0, 1.0)
...
aa_file=args[2]
folder_name=args[3]
nt_input=paste("../primer_design_auto/iupac_nt_input/len_10/nt_", args[1], ".csv", sep = "")
aa_input=paste("./input/", args[2], sep = "")
input_data = read_f(nt_input, aa_input)
col_num <- ncol(input_data$nt_data)
row_num <- nrow(input_data$aa_data)
...
outdir = paste("./results/", folder_name, "/amb2stop/", nt_file, sep = "")
...
for (i in 1:col_num) {
    ...
    outfile=paste(outdir, "/", args[1], ".", i, ".optimize.out", sep = "")
...
nt_csv=paste(outdir, "/", args[1], ".nt_opt.all.csv", sep = "")
nt_revised=paste(outdir, "/", args[1], ".nt_opt.all_revised.csv", sep = "")
...
aa_csv=paste(outdir, "/", args[1], ".aa_opt.all.csv", sep = "")
aa_revised=paste(outdir, "/", args[1], ".aa_opt.all_revised.csv", sep = "")
...
MSE_csv=paste(outdir, "/", args[1], ".MSE_opt.all.csv", sep = "")
MSE_revised=paste(outdir, "/", args[1], ".MSE_opt.all_revised.csv", sep = "")

    MSE_mat <- matrix(output_MSE_all, nrow = col_num, byrow = TRUE)
    MSE_mat <- t(MSE_mat)

    rownames(MSE_mat) <- c(aa_file)
